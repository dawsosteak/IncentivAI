...
Usage examples:
  ...
  # Anthropic
  uv run python cli.py --file urls.xlsx --provider anthropic --model claude-opus-4-5
...
from config import (
    DEFAULT_TEMPERATURE,
    DEFAULT_TRUNCATION_LENGTH,   # was DEFAULT_TRUNCATION in your original — matched to config.py
    DEFAULT_MODEL,               # was MODEL_NAME in your original — matched to config.py
)
...
Providers:
  ...
  google     Google Gemini API (requires GOOGLE_API_KEY)
    ...
    parser.add_argument(
        ...
        type=str,
        default="ollama",
        choices=["ollama", "openai", "anthropic", "google", "gemini"],  # uw_ssec removed
    ...
    parser.add_argument(
        "--truncation",
        type=int,
        default=DEFAULT_TRUNCATION_LENGTH,
        help=f"Max characters of scraped content to send to LLM (default: {DEFAULT_TRUNCATION_LENGTH})"
    )
