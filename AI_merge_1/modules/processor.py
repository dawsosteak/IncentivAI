...
- For sector: Residential, Commercial, Industrial, Agricultural — list all that apply.
...
6. SECTOR — Residential, Commercial, Industrial, Agricultural (all that apply).
...
def process_text(text: str, url: str, temperature: float, model_name: str | None = None):
    ...
    for attempt in range(MAX_RETRIES + 1):
        response = None
        try:
            response = call_ollama(prompt, temperature, model_name=model_name)
            ...
            if not _row_is_grounded(data, text, url):
                ...
                return {
                    "utility_company": None,
                    "programs": [],
                    "summary_of_page": "Extracted fields were not verifiable against the page text.",
                }
